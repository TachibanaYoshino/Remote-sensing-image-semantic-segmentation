...
import argparse
import numpy as np
from net.model6_1 import Net
from config import Config
from data_loader6_1 import *
...
def train(args):
    EPOCHS = c.num_epochs
    filepath = "weights-{epoch:03d}-{val_l0_loss:.4f}.h5"
    model_weights = os.path.join(args['model'], filepath)

    train_numb = 2500 * c.batch_size
    ...
    loss_func = [lovasz_softmax, lovasz_softmax, lovasz_softmax, lovasz_softmax, lovasz_softmax]
    model.compile(optimizer=adam, loss=  loss_func, loss_weights=[10., 0.1, 0.1, 0.1, 0.1])

    modelcheck = ModelCheckpoint(model_weights, monitor='val_l0_loss', save_best_only=False, mode='auto')
    callable = [modelcheck]
...
def args_parse():
    ...
    ap.add_argument("-m", "--model", type=str, default='checkpoint6_1',
